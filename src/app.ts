import { createAppContext, type Env, type RuntimeContext } from "./lib/env";
...
const NOOP_CONTEXT: RuntimeContext = {
  waitUntil() {},
};
...
export function dispatchRequest(
  request: Request,
  env: Env,
  ctx: RuntimeContext = NOOP_CONTEXT,
): Promise<Response> {
  const appCtx = createAppContext(ctx);

  if (matchesRoute(request, "POST", "/api/probe/report")) {
    return handleProbeReport(request, env, appCtx);
  }

  if (matchesRoute(request, "POST", "/api/admin/overrides")) {
    return handleAdminOverride(request, env, appCtx);
  }

  if (matchesRoute(request, "POST", "/api/admin/announcements")) {
    return handleAdminAnnouncement(request, env, appCtx);
  }
  ...
  if (matchesRoute(request, "POST", "/api/admin/services")) {
    return handleAdminCreateService(request, env, appCtx);
  }

  if (matchesRoute(request, "POST", "/api/admin/components")) {
    return handleAdminCreateComponent(request, env, appCtx);
  }

  if (matchesRoute(request, "POST", "/api/admin/catalog/reorder")) {
    return handleAdminReorderCatalog(request, env, appCtx);
  }
  ...
  if (request.method === "PATCH") {
    ...
    if (url.pathname === "/api/admin") {
      return Promise.resolve(createApiNotFoundResponse());
    }
    ...
    if (url.pathname.startsWith("/api/admin/services/")) {
      ...
      if (slug.length > 0 && !slug.includes("/")) {
        return handleAdminUpdateService(
          request,
          env,
          appCtx,
          slug,
    ...
    if (url.pathname === "/api/admin/components") {
      return Promise.resolve(createApiNotFoundResponse());
    }
    ...
    if (url.pathname.startsWith("/api/admin/components/")) {
      ...
      if (slug.length > 0 && !slug.includes("/")) {
        return handleAdminUpdateComponent(
          request,
          env,
          appCtx,
          slug,
